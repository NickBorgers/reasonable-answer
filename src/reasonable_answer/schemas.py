...
class IssueRepair(BaseModel):
    ...
    unrelated field in the same breath — observed in production as attempt 1 failing
    `span_not_verbatim` and attempt 2 failing `category_out_of_scope`. A patch cannot do
    that: everything it does not name is carried over untouched by
    `triage.apply_repairs`, mechanically, with no model in the path.
    """
