...
#: Artwork for a push notification (D-stop-notification). `icon-192.png` rather than the 512: notification
...
def load(base_path: str = "") -> Assets:
    ...
    *manifest names* and the worker *precaches and fetches* that carry the prefix, because a
    ...
    service_worker = (
        ...
        # The notification icon (D-stop-notification). Substituted for the same reason as OFFLINE: it is a
