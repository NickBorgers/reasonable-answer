...
- **Triage validates** every field against the schema before it becomes a defect-task or a
  ...
  than a weaker version of them: the re-ask is fenced and reads "the schema validator
  rejected the output", never "your previous response", and what it fences is a bounded
  `loc`/`msg`/`type` summary (`errors(include_input=False, include_url=False)`), never a
  pydantic `ValidationError`'s default rendering, which echoes the rejected value itself
  (D-validator-error-hygiene). See
