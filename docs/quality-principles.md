...
|---|-----------|---------|----------|
| QP1 | **No LLM ordinal score enters a control decision.** Continue/stop/accept/merge decisions are deterministic functions of bounded categorical facts; severities are mechanical floors, clamped up only. | `controller.py`, `triage.py`, `taxonomy.py` (the `conceptual_conflation` major floor is a mechanical clamp-up-only floor — D-conceptual-conflation; `triage.mechanical_bibliography_issues` mints referential-integrity facts about the bibliography — marker/entry agreement, decided by string comparison over the report's own text — **at** their category floors, so the clamp stays a no-op on them — D-bibliography-integrity), `schemas.py::OrchestratorView`; CI: `aggregate.mjs`, the judge stage | Grove et al. 2000 |
