...
> procedure in [§4](#4-retiring-or-weakening-a-principle) is a graded violation. See
> [D-quality-reviewer](./decisions/D-quality-reviewer.md).
...
that coherently updates code, spec, *and* its decision entry passes its row 12 by
...
|---|-----------|---------|----------|
| QP1 | **No LLM ordinal score enters a control decision.** Continue/stop/accept/merge decisions are deterministic functions of bounded categorical facts; severities are mechanical floors, clamped up only. | `controller.py` (including `select_shipped_index`, which picks the shipped draft under either `review.selection` rule from bounded categorical counts alone — no LLM ranks the rounds, and `latest_unblocked` narrows what the choice reads rather than adding a judgement to it, D-latest-unblocked-selection), `triage.py`, `taxonomy.py` (the `conceptual_conflation` major floor is a mechanical clamp-up-only floor — D-conceptual-conflation; `triage.mechanical_bibliography_issues` mints referential-integrity facts about the bibliography — marker/entry agreement, decided by string comparison over the report's own text — **at** their category floors, so the clamp stays a no-op on them — D-bibliography-integrity; a bibliography the body never cites is one `uncited_claim` minted at its `major` floor from the same string facts — D-uncited-bibliography), `schemas.py::OrchestratorView`; CI: `aggregate.mjs`, the judge stage | Grove et al. 2000 |
| QP2 | **Second witnesses are cross-family.** Agreement adds information only to the extent errors are decorrelated; a same-family model added as an extra witness adds correlated errors, not independence — and even cross-family agreement is partial evidence, never proof. | `config/roster.yaml`; `roles.py::critic_slate` selects at most one model per family and `roles.py::lens_statuses` counts clean families; `config.py::validate_roster_health` warns when a pool cannot supply two families, and re-runs on the reduced roster when `graph.py::_degrade_roster` drops an unprobeable alias or `graph.py::_record_strikes` sidelines a repeatedly failing critic, so either path can only ever *shrink* the eligible family set and never substitute a same-family witness for a missing one (D-degraded-roster, D-failing-critic-sidelined) | Kim et al. 2025; Li et al. 2025; Verga et al. 2024 |
...
| QP5 | **No critique or report prose crosses contexts as instruction.** Review happens in fresh, blind contexts; critic→writer traffic is bounded structural data; the blind orchestrator sees integers and enums. CI reviewers never read each other's findings. | `prompts.py` (including marker-scrubbed `DATA_FENCE`/`DATA_END` interpolations in every direction under D-fence-scrub-all-directions, the untrusted-data fences around writer-read pages, and the separate support pass), `llm.py::structured` (schema-repair feedback is validator-attributed, fenced and marker-scrubbed; rejected input and model-authored extra-key locations are excluded — D-validator-error-hygiene), `triage.py`, `schemas.py` (patch-scoped revision under `revision.mode`/D-scoped-revision and D-claim-scoped-patch narrows the *edit* to named paragraphs and restatements of a fixed claim, not the traffic — critic→writer input stays bounded structural data and the review/context boundary is unchanged), `ops.py` (under `revision.mode: ops` the writer returns operations on the `[S<n>.P<m>]` labels critics already read and a mechanical splice builds the next artifact; the handoff gains only those labels and a task ordinal, and critics read the spliced Markdown in the unchanged rendering — D-ops-revision; the committed production roster selects this mode while the code default remains `patch` — D-ops-revision-rollout); `graph.py::_triage` carries only the prior draft's bounded URL set in `defect_citation_scope` for later adjudication, never that draft's prose (D-dispute-evidence-prior-draft); CI: independent reviewer jobs | Song 2026; Sharma et al. 2023 |
...
| QP7 | **Every loop is capped, and the cap is honored at every entry point.** More iterations do not monotonically improve output; refinement loops adapt to their evaluator. | `controller.py`, `config.py::Budgets` (the rule-13 rewrite valve is a new loop, bounded by `Budgets.rewrite_cap` and unreachable at/beyond the hard cap — D-scoped-revision), `config.py::RevisionConfig.repair` (the census-gated repair turn spends at most `repair_cap` extra writer calls per generation and re-measures rather than looping to a clean verdict — D-census-gated-repair), and `config.py::CallTimeouts` plus the writer, critique, repair, and claim-check call sites in `graph.py`, `critique.py`, and `claimcheck.py` (per-role call-attempt bounds remain inside the capped retry and writer-attempt loops — D-role-call-timeouts); recovery spends **two** capped budgets, not one — `max_resume_attempts` bounds a run failing on its own inputs, and `max_deferred_attempts` bounds both startup deployment refusals (D-deferred-not-abandoned) and mid-run provider-account refusals (D-credit-exhaustion-defers), counted by `registry.consecutive_deferrals` and terminating in the same `abandoned` event; a deferral is cheap, which is an argument for a generous cap and not for an absent one, since an uncapped deferral loop would accumulate runs that never reach a terminal state for anyone to notice; CI: `MAX_CYCLES`, the dispute budget, agent timeouts, the bounded reviewer retry (D-reviewer-retry-transient), and the inherited-verdict classifier that preserves a cycle only for a verified no-new-content case: either a whole-range, tree-identical base resync or an unchanged head equal to its verified review anchor (D-inherit-whole-range, D-inherit-reviewed-anchor; the resync recreation registers the same regional merge driver the fixer's sync uses, so a driver-resolved resync still recreates identically when independently added decision sections and clean shared-region edits are recombined — D-decisions-merge-driver, D-decisions-merge-regions, both retired by D-decision-per-file, which removed the collision they resolved rather than resolving it, so this recreation path is now the plain deterministic `git` merge with no driver in it) | Huang et al. 2024; RG-001/RH-001 lineage in [decisions.md](./decisions.md) |
| QP8 | **Verdicts come from deterministic aggregation of structured findings, never an LLM grading prose.** Expert reviewer agreement is low even among humans; the remedy is structure and mechanical aggregation, not a smarter holistic judge. | `controller.py`, the blind orchestrator; `audition.py` (`judge` computes `fit`/`unfit` from aggregated `Metrics` against `AuditionThresholds`; `CacheEntry.matches`/`rubric_hash` decide which stored `Metrics` a verdict may be read from; measuring paths include `structured_output_mode` in that identity, while the no-spend `cached_judgements`/`enforce_fitness` startup gate deliberately reads across mode drift and reports it separately — D-audition-rubric-identity, D-audition-probe-parity); CI: the judge and the deterministic whole-range/tree-identity gate controlling whether its prior verdict may be inherited, both run from `main` (D-inherit-whole-range; the gate's `docs/decisions.md` recreation stays deterministic under the trusted regional merge driver, which uses `git merge-file` for shared regions and decides only the ordering of independently added decision sections — D-decisions-merge-driver, D-decisions-merge-regions, both retired by D-decision-per-file, which removed the collision they resolved rather than resolving it, so this recreation path is now the plain deterministic `git` merge with no driver in it) | Grove et al. 2000; Beygelzimer et al. 2021; Verga et al. 2024 |
...
**Application — transient reviewer retry (D-reviewer-retry-transient).** The read-only reviewer is
the only caller that opts into two attempts; every other caller retains the one-attempt default. A
retry is allowed only when the first attempt fails within the 600-second fast-failure window, and
any session mount disables it, covering every resume and every invocation that carries session state
out of the container. The retry delay and a full 30-minute second attempt remain bounded by the
reviewer job's 45-minute outer timeout. These attempt, eligibility, elapsed-time, and job bounds are
one QP7-governed loop and must be re-checked together when any of them changes.

**Application — base-moved resync (D-base-moved-resync; inert since D-decision-per-file).**
`sync-open-prs.yml` added a call site for the `docs/decisions.md` merge driver outside any review
cycle. The driver is retired — one file per decision means there is no shared insertion point left
to resolve — so the workflow now finds nothing to push on any PR. The bounds below still describe
its loop, which still runs: two QP7-governed bounds hold it:
...
1. updating the row in this file and recording the change in a `docs/decisions/D-<slug>.md`
   entry, **and**
