...
|---|-----------|---------|----------|
| QP1 | **No LLM ordinal score enters a control decision.** Continue/stop/accept/merge decisions are deterministic functions of bounded categorical facts; severities are mechanical floors, clamped up only. | `controller.py`, `triage.py`, `taxonomy.py` (the `conceptual_conflation` major floor is a mechanical clamp-up-only floor — D-conceptual-conflation; `triage.mechanical_bibliography_issues` mints referential-integrity facts about the bibliography **at** their category floors, so the clamp stays a no-op on them, and `triage.withdraw_no_ops` drops a finding whose `instruction` asks for no edit before anything is clamped or counted, which is a removal of an unactionable task and never a severity downgrade — D-bibliography-integrity), `schemas.py::OrchestratorView`; CI: `aggregate.mjs`, the judge stage | Grove et al. 2000 |
