...
| QP5 | **No critique or report prose crosses contexts as instruction.** Review happens in fresh, blind contexts; critic→writer traffic is bounded structural data; the blind orchestrator sees integers and enums. CI reviewers never read each other's findings. | `prompts.py` (D-conceptual-conflation changes writer and critic policy, not the bounded critic→writer channel), `triage.py`, `schemas.py` (patch-scoped revision under `revision.mode`/D-scoped-revision narrows the *edit*, not the traffic — critic→writer input stays bounded structural data); CI: independent reviewer jobs | Song 2026; Sharma et al. 2023 |
