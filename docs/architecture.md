...
| **triage** | this tick's critic and mechanically authored `Issue[]` (D-bibliography-integrity) (minus **upheld-adjudication suppressions**, D-writer-disputes) | `OrchestratorView` + `DefectList` | none — **mechanical** | deterministic |
...
SIGKILL, none of which it controls. A run is a long-running operation, so finishing one
inside the grace window is not achievable and is not attempted.
...
- **Transient-failure posture (D-provider-retry):** every retry waits — exponential with jitter, bounded by
  ...
  modes countable without treating volatile provider prose as an interface. Two classes are the
  graph's own rather than `llm`'s: `empty_report` (the call succeeded and the model answered with
  whitespace) and, under `revision.mode: ops`, `malformed_ops` (the model answered, but with no
  operation the splice could apply — D-ops-revision); both move the attempt to the next pool
  member. The two capability
