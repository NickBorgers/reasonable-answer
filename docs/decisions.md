...
adversarial-review round headings, every pre-slug `## D39 — …` section — and keying on slugs would
...
| 20 | `config/roster.yaml` audition NOTE | said `gemma4` on evidence is "at position 3 and still unmeasured" | the pool is two entries, `gemma4` is at position 2, and it was measured in the same 2026-08-02 audition round (`marginal`) |
