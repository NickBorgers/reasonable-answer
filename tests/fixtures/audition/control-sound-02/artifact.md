...
- Unsubsidized utility-scale solar and onshore wind sit well below new nuclear per MWh in US conditions, with the nuclear range reflecting recent Western construction experience [1].
...
"Cost-competitive" is not well defined across technologies that deliver different products at different times. Levelized cost divides lifetime cost by lifetime output and is silent on when that output arrives, which Hirth shows is not a minor omission: the market value of variable generation falls as its share of the system rises, because its output correlates with output already there [3]. A comparison made at one penetration level therefore does not transfer to another.

This report accordingly gives the levelized answer and the system answer separately rather than reconciling them into a single number, and the two sections below should be read as answers to two different questions.
...
Sepulveda and colleagues model whole decarbonised systems rather than individual plants, and find a cost-reducing role for firm low-carbon resources across much of their scenario space [4]. On that framing the levelized comparison understates nuclear's contribution.

Their own sensitivity analysis qualifies it: the size of the effect depends heavily on assumed storage costs, and cheap enough storage shrinks it substantially [4]. Sustained storage cost declines would therefore erode the system-level premium in the same direction that a successful Western build would narrow the levelized gap.

## Sources

1. Lazard (2024). *Levelized Cost of Energy+*, version 17.0.
