// Guards the one schema rule that has broken the review pipeline four times.
//
...
// confidence — every reviewer prompt's 0.7 ladder is literally "same finding, other array".
// Both items set `additionalProperties: false`, so a field a reviewer puts on a note that
// only the blocker admits fails validation, and a failed artifact loses EVERY finding that
// reviewer had, not just the offending one. It then reads downstream as "no reviewer
// artifact", which the judge fails closed on — correctly, but with a diagnostic that points
// nowhere near the actual cause.
//
// That has now happened with `id`, `decision_ref` (#29), `category` (#35), and `confidence`
// (#75, then again across six PRs in one afternoon, #131). Each was fixed by admitting one
// field, which is why it kept recurring: the fix closed an instance, not the class. This test
// closes the class by asserting the two property sets stay equal, so admitting a field to one
// array is a build error until it is on both.
//
...
//
// See docs/decisions.md D-reviewer-confidence-field.
...
// it is *required* on every fix_suggestions[] entry and named in every prompt's ladder. A
// reviewer with that field in its output contract attaches it to the finding too.
  ...
  for (const field of descriptive) {
    assert.ok(
      ...
      `\`${field}\` is required on fix_suggestions[] but not admitted on blocking_issues[]. ` +
        `A reviewer reasoning in terms of it will attach it to the finding; see #131.`,
    );
    assert.ok(
