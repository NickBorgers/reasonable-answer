...
**The web interface authenticates on a trusted header, not a verified one (D-identity-header).** Identity comes
...
`GET`s under `/runs/`, which are deliberately public (D-id-as-credential) — holding a run id is the credential
...
the port directly can claim to be anyone — is a **documented accepted risk** in D-identity-header and
...
- "The identity header is trusted, not verified." Documented accepted risk (see Role, D-identity-header). Only
