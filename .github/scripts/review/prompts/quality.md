...
| QP3 | The CI panel itself stays family-diverse, and names what it runs | An `agent:` or `model:` flip makes the review panel single-family; a new role's family is chosen with no stated reason; a role becomes same-family with the role it primarily deconflicts with; a role reaches `review-agent-run` with no model pinned, or a pin is removed so the role falls back to a CLI default. |
